...
For instance, we compute text variants of our base colors. So locally overriding the `colors.primary` color will automatically cascade to the `colors.textPrimary`. You can however, directly override computed values as well by passing a custom value to this theme variable.
